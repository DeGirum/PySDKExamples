...
#     parser.add_argument('--save_pred_path', type=str, help='save the predictions as a json')
    ...
    parser.add_argument('--save_mAP_path',type=str, default='./mAP_results.csv', help='saving the results to a csv file')
    parser.add_argument('--cloud_zoo_url',type=str, default='public', help='model zoo url from the Degirum cloud zoo (https://cs.degirum.com/)')
    parser.add_argument('--cloud_token',type=str, default='dg_AMJjLaPG4YBYPcCDfUMKPJbrDmSoJSjLJruYw', help='generate a token from My Tokens in https://cs.degirum.com/')
...
def main(opt):
    ...
    models = dg.connect(dg.CLOUD, "https://cs.degirum.com/degirum/" + opt.cloud_zoo_url, opt.cloud_token) # get all the models from the model zoo
    
    with open('./coco_configs/classmap_config.json') as config:
        classmap_categories=json.load(config)
        
#     with open('./models_test.json' , "r") as json_data:
#         model_zoo=json.load(json_data)
#     models_list = models[opt.cloud_zoo_url]
    ...
    mAP_list = []
    models_list = models.list_models(model_family="yolo")
#     print (models_list)
...
#         eval_obj.summarize()
        ...
        with open(opt.save_mAP_path, 'w', encoding='UTF8', newline ='') as f:
